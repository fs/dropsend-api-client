...
class OAuthToken {
  ...
  function to_string() {
    global $oauth_util;
    return "oauth_token=" .
           $oauth_util->urlencode_rfc3986($this->key) .
           "&oauth_token_secret=" .
           $oauth_util->urlencode_rfc3986($this->secret);
  }
...
class OAuthSignatureMethod_HMAC_SHA1 extends OAuthSignatureMethod {
  ...
  function build_signature($request, $consumer, $token) {
    ...
    $key_parts = $oauth_util->urlencode_rfc3986($key_parts);
...
class OAuthRequest {
  ...
  function OAuthRequest($http_method, $http_url, $parameters=NULL) {
    ...
    $parameters = array_merge( $oauth_util->parse_parameters($query), $parameters);
  ...
  function get_signable_parameters() {
    ...
    return $oauth_util->build_http_query($params);
  }
  ...
  function get_signature_base_string() {
    ...
    $parts = $oauth_util->urlencode_rfc3986($parts);
  ...
  function to_postdata() {
    global $oauth_util;
    return $oauth_util->build_http_query($this->parameters);
  }
  ...
  function to_header($realm=null) {
    global $oauth_util;
	if($realm)
      $out = 'Authorization: OAuth realm="' . $oauth_util->urlencode_rfc3986($realm) . '"';
    else
    ...
    foreach ($this->parameters as $k => $v) {
      ...
      if (is_array($v)) {
        /* exception */ die('Arrays not supported in headers');
      }
      $out .= ',' .
              $oauth_util->urlencode_rfc3986($k) .
              '="' .
              $oauth_util->urlencode_rfc3986($v) .
              '"';
    }
    return $out;
  }
   ...
   */
  function oauthrequest_from_consumer_and_token($consumer, $token, $http_method, $http_url, $parameters=NULL) {
	global $oauth_util;
    @$parameters or $parameters = array();
    $defaults = array("oauth_version" => '1.0',
                      "oauth_nonce" => $oauth_util->generate_nonce(),
                      "oauth_timestamp" => $oauth_util->generate_timestamp(),
                      "oauth_consumer_key" => $consumer->key);
    if ($token)
      $defaults['oauth_token'] = $token->key;

	//var_dump($parameters, $defaults);
   ...
   */
  function oauthrequest_from_request($http_method=NULL, $http_url=NULL, $parameters=NULL) {
    global $oauth_util;
    ...
    if (!$parameters) {
      ...
      $request_headers = $oauth_util->get_headers();

      // Parse the query-string to find GET parameters
      $parameters = $oauth_util->parse_parameters($_SERVER['QUERY_STRING']);
      ...
      if ($http_method == "POST"
        ...
        $post_data = $oauth_util->parse_parameters(
      ...
      if (@substr($request_headers['Authorization'], 0, 6) == "OAuth ") {
        $header_parameters = $oauth_util->split_header(
...
class OAuthServer {
  ...
  function get_version(&$request) {
    ...
    if (!$version) {
      ...
      // Chapter 7.0 ("Accessing Ressources")
      $version = '1.0';
    }
    if ($version !== $this->version) {
      /* exception */ die("OAuth version '$version' not supported");
    }
    return $version;
  }
  ...
  function get_signature_method(&$request) {
    ...
    if (!$signature_method) {
      // According to chapter 7 ("Accessing Ressources") the signature-method
      // parameter is required, and we can't just fallback to PLAINTEXT
      /* exception */ die('No signature method parameter. This parameter is required');
    }
  ...
  function get_consumer(&$request) {
    ...
    if (!$consumer_key) {
      /* exception */ die("Invalid consumer key");
    }

    $consumer = $this->data_store->lookup_consumer($consumer_key);
    if (!$consumer) {
      /* exception */ die("Invalid consumer");
    }
  ...
  function get_token(&$request, $consumer, $token_type="access") {
    ...
    if (!$token) {
      /* exception */ die("Invalid $token_type token: $token_field");
    }
    return $token;
  }
  ...
  function check_signature(&$request, $consumer, $token) {
    ...
    if (!$valid_sig) {
      /* exception */ die("Invalid signature");
    }
  }
  ...
  function check_nonce($consumer, $token, $nonce, $timestamp) {
    ...
    if ($found) {
      /* exception */ die("Nonce already used: $nonce");
    }
  }
  ...
  function split_header($header, $only_allow_oauth_parameters = true) {
    ...
    while (preg_match($pattern, $header, $matches, PREG_OFFSET_CAPTURE, $offset) > 0) {
      ...
      if (preg_match('/^oauth_/', $header_name) || !$only_allow_oauth_parameters) {
        $params[$header_name] = $this->urldecode_rfc3986($header_content);
      }
